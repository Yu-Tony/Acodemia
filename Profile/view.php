...
$result = $conn->query("SELECT image_url FROM testtable WHERE id = '".$resultado."'"); 
...
<?php if($result->num_rows > 0){ ?> 
    <div class="gallery"> 
        <?php while($row = $result->fetch_assoc()){
            ...
            {?> 
                <img alt="Imagen de perfil" class="rounded-circle p-1 bg-primary" width="110"  src="data:image/jpg;charset=utf8;base64,<?php echo base64_encode($row['image_url']); ?>" /> 
            <?php } 
            else{?> 
